...
def harmonicDetection(pfreq, pmag, pphase, f0, nH, hfreqp, fs, harmDevSlope=0.01):
	...
	while (f0>0) and (hi<nH) and (hf[hi]<fs/2) and (pfreq!=[]):          # find harmonic peaks
		...
		if (dev1<threshold):         # accept peak if deviation is small
			hfreq[hi] = hf[hi]                           # plug in known frequency value
			hmag[hi] = pmag[pei]                             # harmonic magnitudes
			hphase[hi] = pphase[pei]                         # harmonic phases
		#else:         # not accept peak if deviation is small
		#	hfreq[hi] = np.zeros(np.shape(pfreq[pei]))                           # harmonic frequencies
		#	hmag[hi] = np.zeros(np.shape(pmag[pei]))                             # harmonic magnitudes
		#	hphase[hi] = np.zeros(np.shape(pphase[pei]))
...
def harmonicModelAnal(x, f0, fs, w, N, H, t, nH, minf0, maxf0, f0et, harmDevSlope=0.01, minSineDur=.02):
	...
	while pin <= pend-H:
		...
		hfreq, hmag, hphase = harmonicDetection(ipfreq, ipmag, ipphase, f0[f0_idx], nH, hfreqp, fs, harmDevSlope)  # find harmonics
		is_all_zero = not np.any(hfreq)
		if is_all_zero:
			f0_new[f0_idx] = 0.0
		hfreqp = hfreq
	...
	return xhfreq, xhmag, xhphase, f0_new
