...
def get_occurrences(ss, pitch, l, n_occ, mask):
    ...
    mask = np.array([x for x in mask if not x >= len(stmass)])
...
def get_motif_clusters(mp, pitch, lengths, top_n, n_occ, exclusion_mask, thresh=None, min_occ=None):
    ...
    while n_returned_groups < top_n:      
        ...
        this_mask = np.concatenate((mask, np.where(coparr==np.Inf)[0]))
        this_mask = np.array(sorted(list((set(this_mask)))))
        pats, dists = get_occurrences(ix, pitch, l, n_occ, this_mask)
...
def get_exclusion_mask(mask, lengths, exclusion_funcs, path=None):
    ...
    to_return = []
    for i in tqdm.tqdm(list(range(len(mask)))):
        to_return.append(any(er(mask[i:int(i+lengths[i])]) for er in exclusion_funcs))
